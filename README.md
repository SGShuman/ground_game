...
### Introduction
The folk wisdom of the 2008 and 2012 presidential elections is that the Democrat's superior 'ground game' handed them the crucial swing states needed for an electoral victory.  By mobilizing 786 field offices <a href=http://www.mischiefsoffaction.com/2013/01/the-ground-game-in-2012.html>[1]</a> located in counties in battle ground states, grassroots organizers were able to get out the vote and just edge Obama over the top.  But how much truth is there to this traditional narrative?
...
These analyses have typically relied on factors like median income, unemployment, demographic factors and historical voting patterns [<a href=http://apr.sagepub.com/content/42/3/529>3</a>, <a href=http://www.acweinschenk.net/uploads/1/7/3/6/17361647/psq12213.pdf>5</a>] to regress the share of the two party vote each candidate receives.  [Two party vote share is the percentage of votes cast for a single party divided by the total votes cast for Democrats and Republicans.]

While similar features will be used in my approach, I regress instead the single party vote as a percentage of voting age population.  Recent analysis <a href=http://static1.1.sqspcdn.com/static/f/864938/25350207/1408716516677/Allen+et+al.+APSA+2014.pdf?token=8SQWbYH63TzRnaVXi9JXF1mWlTY%3D>[6]</a> has shown that the effect of field office placement is mostly to encourage party aligned voters to actually vote on election day.  Ground game does not flip voters.  Thus, typical analysis of two party vote share, which depends on both the number of Democrats and Republicans, might miss part of the effect.  Instead, regressing single party vote as a percentage of voting age population removes the dependency on the other party's vote and allows us to analyze each strategy in a vacuum.

I find that counties with a historical voting bias have a larger effect from field office placement.  I present a strategy to maximize this effect while balancing state effects. In addition I show that for the current political reality, only a few states are susceptible to changing their electoral votes keeping all else constant.

### Methodology
In order to understand and quantify the effect of field office placement, I gathered data from a large number of sources to fit a linear model.  Coefficients from that linear model can estimate the effect of a field office on the single party vote. Using data and field office placement at the county level gives a reasonably sized unit of analysis.  The exogenous variables besides field office placement bear out a detailed explanation and some visualizations of exploratory data analysis.

#### Demographic Factors
Two education factors were used: <a href=http://www.ers.usda.gov/data-products/county-level-data-sets.aspx>[7]</a> the percentage of adults without a high school diploma and the percentage of adults with a bachelor's degree or higher.  In addition the county unemployment rate <a href=http://www.ers.usda.gov/data-products/county-level-data-sets.aspx>[7]</a>  was also used.

From <a href=http://www.thearda.com/Archive/Files/Downloads/RCMSCY10_DL2.asp>[8]</a> nearly 15,000 columns on the rate of religious denomination participation within county populations, I used Non-negative Matrix Factorization (NMF) to reduce these columns to two latent 'topics'.  These 'topics' basically subdivide counties into two groups.  If there are different concentrations of Democratic and Republican leaning counties in these topics, then this feature will allow us to tell Democratic and Republican counties apart.

In fact, by looking at just one of the NMF topics a good separation between red and blue counties is apparent as shown by **Figure 1**.  The blue counties are much further concentrated to the left side of the chart.  Many more of the blue counties are concentrated at low values of this feature while Republican counties are concentrated at larger values.
...
*81 historically Republican counties voted Democrat while only 5 historically Democratic counties voted Republican.*
...
###### Figure 3: Change in Two Party Vote Share Histogram 
*There was a negative change in vote share for 414 Democratic Counties, less than 250 Republican Counties became less extreme.*
...
#### State Level Effects
To counter state level fixed effects, I used the 2012 state level partisan turnout <a href=http://bipartisanpolicy.org/library/2012-voter-turnout/>[9]</a> as a feature as well.  This allows the model to account for state by state differences in baseline voting rate. 

#### Office Data and the Interaction Feature
...
Obama placed anywhere from 1 to 21 offices in a county (with by far most having one office), so I used the presence of 1 office or 2+ offices as categorical features.  I also interacted (multiplyed) the Cook Index with a boolean variable indicating the presence of any number of offices.  This means that the number of votes expected from placing a field office in a county will depend on both the number of offices placed and the Cook Index.  In the end, counties with larger Cook Indexes (indicating more partisan historical voting) had a larger effect from field office placement.

I won't include a map here (I'm sure you can picture a map of the US in your head), but please refer to <a href='http://blogs.lse.ac.uk/usappblog/2014/05/12/when-placed-strategically-campaign-field-offices-can-be-very-important-in-turning-battleground-states-during-presidential-elections/'> Joshua Darr's and Matthew Levendusky's maps </a> for additional clarity <a href=http://apr.sagepub.com/content/42/3/529>[3]</a> on field office placement.  Cook Indexes also follow the typical pattern of showing population centers as blue and rural counties as red <a href=https://en.wikipedia.org/wiki/Cook_Partisan_Voting_Index>[10]</a>.

### Linear Model Results
As found in previous papers, I established Democratic and Republican field offices offer starkly different results.  While the regression on Democratic turnout provides more statistical certainty, the Republican effect can still be quantified.  Democratic field offices in my model were responsible for 2.0M votes nationally, with 0.5M for Republicans.
...
| Religion NMF Topic 1                  | 0.0091     | 0.001    | 0.000 |
| Religion NMF Topic 2                  | 0.0063     | 0.000    | 0.000 |
...
*The factors and coefficients from the linear model predicted Democratic turnout.  All coefficients are significant.  Because the data are not normalized, the coefficients are not directly comparable.  In addition, to understand the effect of a field office in terms of votes added, more computation is needed.  See **Simulation in Action** *

Regressing Republican turnout with a similar adjusted R2 to Democrats shows a strong fit but not a strong statistical certainty for field office placement having an effect.
...
| Religion NMF Topic 1                  | 0.0003     | 0.001    | 0.700 |
| Religion NMF Topic 2                  | 0.0072     | 0.001    | 0.000 |
...
###### Figure 6: State Level Effects
*The left figure shows the effect of putting a field office in every county of that state for both Democrats and Republicans.  The right shows the average net effect of this strategy.  Error bars indicate a maximum and minimum to the possible net effect of ground games from both parties.  Notice that most states with a Republican leaning net effect do not have a net Democratic effect scenario (check out the error bars), while all Democratic leaning states do.*

The simulation will only place a limited number of offices, so any result within the error bars in **Figure 6** above is available.  Even more extreme results are available as well if one party ignores a state and the other floods it.  In fact, since we can expect some statistical variation, the simulation can identify all the states who could possibly flip in any situation. 

### Simulation and Strategy

I simulated the 2012 election 1000 times with the goal of identifying scenarios that cause states to flip.
...
3. Place 800 offices in counties around the country.  Place more offices in states with closely contested elections.  Place either one or two offices, determined randomly.  *Different office placements will be chosen each time.*
4. Draw from a normal distribution for each county to determine the effect of a field office in every county that got one or two offices.
...
* interaction_effect is drawn from similar normal distribution multiplied and by county Cook Index  
...
**Figure 7** shows the battleground states under traditional understanding. The highlighted bubbles show the closest states.

<div>
    <a href="https://plot.ly/~SGShuman/81/" target="_blank" title="Swing States - Traditional Understanding" style="display: block; text-align: center;"><img src="https://plot.ly/~SGShuman/81.png" alt="Swing States - Traditional Understanding" style="max-width: 100%;width: 600px;"  width="600" onerror="this.onerror=null;this.src='https://plot.ly/404.png';" /></a>
    <script data-plotly="SGShuman:81"  src="https://plot.ly/embed.js" async></script>
</div>

###### Figure 7: Swing States with the Traditional View  
*Highlighted states reveal the typical battleground states.  The highlighted bubbles indicate which states have single party vote percentages for each party within 5% of each other.  The size of the bubble is the number of electoral votes.  The color reveals the winner in the 2012 election.  Click to enable hover.*

Applying what the results about the possible range of the field office effect in each state, only the highlighted states in **Figure 8** could possibly switch.  Conventional wisdom on swing states is upheld but limited.

<div>
    <a href="https://plot.ly/~SGShuman/83/" target="_blank" title="Swing States from Field Office Placement" style="display: block; text-align: center;"><img src="https://plot.ly/~SGShuman/83.png" alt="Swing States from Field Office Placement" style="max-width: 100%;width: 600px;"  width="600" onerror="this.onerror=null;this.src='https://plot.ly/404.png';" /></a>
    <script data-plotly="SGShuman:83"  src="https://plot.ly/embed.js" async></script>
</div>
...
First let's discuss the two factors are important in adding votes: the county size, and the **office effect**.  The county size is the most important factor, the larger a county is, the more votes are added to the state due to field office placement.  If that county contains a large percentage of the state population, then this can dominate the states total added votes from field offices.  The other factor is the office effect which is the magnitude of the increase in votes after being drawn from the county by county normal distributions defined above.  County size does not change simulation to simulation, but the size of office effect does because of its random nature.
...
**Figure 9** shows this.  The size of the bubble is the size of the office effect while the height is the percentage of the population that lives in that county.  Counties with higher positions are more influential in terms of population and larger bubbles indicate larger average office effect.
...
However, this assumes that both Democrats and Republicans blanket every county with field offices.  What these findings suggest is a more subtle strategy.  Closer to election day, the probable state results will be known with much more precision due to polling and models like <a href=fivethirtyeight.com> FiveThirtyEight <a>.  By choosing states that are very close near election day, the field office strategy can have a large effect.  By placing offices in historically biased counties with large populations, each party can limit the resources spent on each state while maximizing votes gained.  For Democrats this can essentially mean target cities while for Republicans it may suggest a more local approach of finding strongly partisan pockets in densely populated areas.

Overall given limited resources, only states that are statistically close enough to be turned should be considered.  Given **Figure 6**, we can now find those.

### Next Steps
The unit of analysis for this research was the county level.  However, as counties are contiguous, it is not at all clear that this is an appropriate unit of analysis (although it is a convenient one).  Next steps could focus on both a more granular approach and the causes or predictors of ineffectiveness in a field office.  Clues as to what could make the Republican ground game more effective could be very important.

The simulation also assumed an even strategy between Democrats and Republicans.  In other simulations, even giving Dems no offices and Republicans thousands, we see similar results.  Not all states can be flipped in the current political climate.  Further analysis should focus on this asymmetry to find more innovative strategies for each party.
