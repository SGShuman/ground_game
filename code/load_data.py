...
class Census_Data_Loader(object):
	...
	def load_columns(self):
	    ...
	    suffix: E is total #, PE is percent, M is margin of error, PM percent margin
		'''

		if self.api_type == 'summary':
			cols_link = 'http://api.census.gov/data/'+self.year+'/acs5/variables.json'
		else:
			cols_link = 'http://api.census.gov/data/'+self.year+'/acs5/profile/variables.json'
		...
		# census_cols.columns = ['name', 'label', 'concept']
        # Is the data type invidual (0) or household (1)
		# census_cols['household'] = census_cols['label'].apply(lambda x:
		#                             1 if 'HOUSEHOLD' in x else 0)
	...
	def get_one_col(self, col_name='NAME'):
		...
		if self.api_type == 'summary':
			search = 'http://api.census.gov/data/'+self.year+'/acs5?get=%s&for=county:*' % (col_name)
		else:
			search = 'http://api.census.gov/data/'+self.year+'/acs5/profile?get=%s&for=county:*' % (col_name)
	...
	def get_all_cols(self, col_list, start=0):
		...
		for col in col_list[start:]:
			...
			if (its % 500 == 0) & (its > 0):
				with open('data/summary_census/summary_census_cols_%s.csv' % its, 'w') as f:
...
def get_county_from_zip_fast(zip_code_series):
	...
	for zip_code in zips:
		...
		try: # This step can take 300 ms per zip code
			zip_county_dict[zip_code] = geonameszip.lookup_postal_code(zip_code[:5], 'US')['county']
		except:
...
def load_expenditures(path='data/raw_data/expenditures/'):
	...
	report_type_mask = temp_df['RPT_TP'].apply(lambda x: (x[0]=='M') & (x[1]!='Y'))
	
	temp_df = temp_df[(obama_mask | romney_mask) & year_mask & report_type_mask].copy()
	
	temp_df['candidate'] = temp_df['CAND_ID'].apply(lambda x: get_cand_from_id(x, match=obama_id, mod_type='dem'))
