...
class OnePartyStrat(object):
    ...
    def set_params(self, one_coef, two_coef, int_coef,
                         one_std, two_std, int_std):
    	'''Reset the office and interaction coefficients

        int_coef should be negative for Republicans'''
        self.one_coef, self.two_coef = one_coef, two_coef
        self.int_coef = int_coef
        self.one_std, self.two_std = one_std, two_std
    ...
    def place_offices(self, weights):
        ...
        while counter > 0:
            choice = np.random.choice([1, 2])  # randomly choose 1 or 2 offices
            if counter == 1:  # if only one office left, choose 1
                choice = 1

            # place choice in the next county chosen by the weighted sample
    ...
    def _get_vote_increase(self, num_offices, county):
    	'''Return the number of votes added to a county by offices'''
        # Get office effect
        if num_offices == 1:
            office_effect = np.random.normal(loc=self.one_coef,
                                             scale=self.one_std)
        elif num_offices == 2:
            office_effect = np.random.normal(loc=self.two_coef,
                                             scale=self.two_std)
        else:
            return 0

        # Get effect of office and cook score interaction
        cook_effect = np.random.normal(loc=self.int_coef,
                                       scale=np.absolute(self.int_std))
    ...
    def get_av_increase(self):
        ...
        return self.df[['NAME', 'state_abbr', 'votes_predicted',
                        'max_vote_increase', 'av_vote_increase',
                        'av_increase perc of Predicted', 'vote_effect',
...
class Simulation(object):
    ...
    def __init__(self, num_sims, electoral_df, dem_strat, rep_strat):
        ...
        self.electoral_dict = electoral_df.set_index('state_abbr')\
        ...
        self.states = electoral_df[['state_abbr']].copy()\
    ...
    def _calc_weights(self):
        ...
        grouped_states = self.dem_strat.df\
                             .groupby('state_abbr', as_index=False)\
                             .sum()

        states_with_weight = grouped_states[['state_abbr']].copy()
        # Small distances have higher weights
        weights = 1. / np.absolute(weights)
        states_with_weight['weights'] = weights.values
        temp_df = pd.merge(self.dem_strat.df[['state_abbr']],
                           states_with_weight, on='state_abbr')
    ...
    def _calc_elect_votes(self):
        ...
        dem_votes = self.dem_strat.df[['state_abbr', 'total_votes']]\
                        .groupby('state_abbr').sum()
        rep_votes = self.rep_strat.df[['state_abbr', 'total_votes']]\
                        .groupby('state_abbr').sum()
        ...
        for state in state_winner.iterkeys():
            if state_winner[state]:
                ...
                # If state is won for the party, count those field offices
                self.counties['dem'] += self.dem_strat\
                                            .df['num_offices_sim'] * mask
            else:
                rep_elect += self.electoral_dict[state]
                self.states['rep'][state] += 1
                mask = self.rep_strat.df['state_abbr'] == state
                self.counties['rep'] += self.rep_strat\
                                            .df['num_offices_sim'] * mask
    ...
    def get_strategy(self, thresh=.45, mod_type='dem'):
        ...
        # mod_type column contains all places
        # offices were placed in winning simulation by party
        ...
        # Place 2 offices by total offices placed until
        # the threshhold is reached, then place one office
        counter, iterator = 0, 0
        while counter < num_offices:
            idx = most_placed_idxs[iterator]
            # Only place 2 offices if there are two left
            if counter - num_offices == 1:
                strategy[idx] = 1
                counter += 1

            # Place 2 offices until the threshold is reached
    ...
    def get_swing_states(self):
        ...
        self.states['swing'] = np.absolute(self.states['dem'] -
                                           self.states['rep'])
    ...
    def plot_swing(self, plots_on=False):
    	...
    	self.states['color'] = self.states['color']\
                               .apply(lambda x: 'b' if x else 'r')
    	output = self.states.drop(['AK', 'SD'], axis=0)
    	print output  # .sort_values(by='color').head(50)
    	if plots_on:
            sizes = (1. / output['swing']) * 1000
            plt.scatter(output['dem'], sizes,
                        s=100, alpha=.4, c=self.states['color'])
